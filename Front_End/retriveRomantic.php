<?php
  $romanticret=mysqli_query($con,"select * from romantic order by book_id desc limit 6");
  if(mysqli_affected_rows($con)>0){
    while($data=mysqli_fetch_row($romanticret)){
      ?>
...
<div class="row cource-list-agile pt-4">
				...
				<div class="buttons-w3ls">
					<a class="btn button-cour-w3ls text-white" href="romanticdetail.php?id=<?php echo $data[0]?>" role="button">Learn More</a>
					<a class="btn bg-dark text-white" href="buy_Romantic.php?id=<?php echo $data[0]?>" role="button">Buy Now</a>
				</div>
			</div>
