<?php
  $scifiret=mysqli_query($con,"select * from scifi order by book_id desc limit 6");
  if(mysqli_affected_rows($con)>0){
    while($data=mysqli_fetch_row($scifiret)){
      ?>
...
<div class="row cource-list-agile pt-4">
				...
				<div class="buttons-w3ls">
					<a class="btn button-cour-w3ls text-white" href="scifidetail.php?id=<?php echo $data[0]?>" role="button">Learn More</a>
					<a class="btn bg-dark text-white" href="buy_Scifi.php?id=<?php echo $data[0]?>" role="button">Buy Now</a>
				</div>
			</div>
